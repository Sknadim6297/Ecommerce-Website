  ...
  return (
    <div className='flex gap-x-4 py-2 lg:px-6 border-b border-gray-200 w-full font-light text-gray-500'>
      ...
      <div className='w-full flex flex-col'>
        ...
        <Link className='text-sm font-medium max-w-[240px] text-primary hover:underline' to={`/product/${id}`}>{title}
        </Link>
        <div onClick={()=>{
          removeFromCart(id);
        }}  className='text-xl cursor-pointer'>
          <IoMdClose className='text-gray-500 hover:text-red- transition'/>
        </div>
    </div>
    <div className='flex gap-x-2 h-[36px]'>
      ...
      <div className='flex-1 h-full flex justify-center items-center cursor-pointer' onClick={()=>increseAmount(id)}>
      <IoMdAdd />
      </div>
      </div>
